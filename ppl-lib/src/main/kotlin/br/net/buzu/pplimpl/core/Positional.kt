...
fun positionalParse(text: String, metadata: StaticMetadata, metaType: MetaType): Any?{
    return when (metadata.kind()){
        Kind.ATOMIC -> atomicParse(text, metadata.info(), metaType.valueMapperOf(metadata.info()))
        Kind.ARRAY -> arrayParse(text, metadata.info(), metaType.typeAdapter, metaType.valueMapperOf(metadata.info()))
